...
class RayNPUExecutor(RayGPUExecutor, NPUExecutor):
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup",
        ...
        if (self.parallel_config.tensor_parallel_size == 1
            ...
            num_npus = self.cache_config.npu_memory_utilization
        else:
        ...
        worker_node_and_npu_ids = self._run_workers("get_node_and_gpu_ids",
                                                    use_dummy_driver=True)

        node_workers = defaultdict(list)  # node id -> list of worker ranks
        node_npus = defaultdict(list)  # node id -> list of npu ids

        for i, (node_id, npu_ids) in enumerate(worker_node_and_npu_ids):
        ...
        }, ) for (node_id, _) in worker_node_and_npu_ids]
        ...
        init_worker_all_kwargs = [
            ...
            ) for rank, (node_id, _) in enumerate(worker_node_and_npu_ids)
        ]
...
class RayXPUExecutorAsync(RayNPUExecutor, RayGPUExecutorAsync):
