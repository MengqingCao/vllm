...
class BlockTracker:
    ...
    用于跟踪前缀缓存分配器内块的状态，每个block对应一个tracker
    active: 活跃
    last_accessed: 上次用的时间
    computed: 计算过的
    """
    ...
    def reset(self):
        """
        复位，last_accessed置为-1，computed置为false
        """
    ...
    def enable(self):
        """
        使能，要求active是非活跃的，然后把activate置为活跃，复位
        """
    ...
    def disable(self):
        """
        不使能，要求active是活跃的，然后把activate置为非活跃，复位
        """
        assert self.active
        self.active = False
        self.reset()
...
class PrefixCachingBlockAllocator(BlockAllocator):
    ...
    def allocate_immutable_block(self,
        ...
        分配具有给给定 token ID 的不可变块，如果可能则重用缓存的块。

        参数：
            prev_block (Optional[Block])：序列中的前一个块。
            token_ids (List[int])：要存储在块中的 token ID。
...
class PrefixCachingBlock(Block):
    ...
    def append_token_ids(self, token_ids: List[int]) -> None:
        ...
        将给定的 token ID 附加到 block，如果 block 变满了，将区块注册为不可变。

        参数：
            token_ids (List[int])：要附加到 block 的 token ID。
        """
        ...
        # native block 处理 写时复制，内存管理策略，推迟内存复制直到实际写入时进行
        ...
        # 如果 content hash 存在，则可以使块不可变。向分配器注册 self，可能会替换 physical block 索引。
